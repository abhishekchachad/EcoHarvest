...
const CartPage = () => {
  ...
  const updateQuantity = async (productId, quantity) => {
    ...
    try {
      await axios.put(`https://ecoharvest-backend.vercel.app/api/cart/${productId}`, {
        userId,
  ...
  const removeItem = async (productId) => {
    try {
      await axios.delete(`https://ecoharvest-backend.vercel.app/api/cart/${productId}`, {
        data: { userId },
      });
      fetchCart();
    } catch (err) {
          ...
          <tbody>
            {cart.map((item) => (
              <tr key={item.product_id}>
                ...
                <td>
                  <img
                    src={`https://ecoharvest-backend.vercel.app/${item.image_url}`}
                    alt={item.name}
                    width="60"
                    height="60"
